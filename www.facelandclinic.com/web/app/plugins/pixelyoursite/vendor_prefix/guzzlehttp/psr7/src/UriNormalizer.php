...
final class UriNormalizer
{
    ...
    public static function normalize(\PYS_PRO_GLOBAL\Psr\Http\Message\UriInterface $uri, $flags = self::PRESERVING_NORMALIZATIONS)
    {
        ...
        if ($flags & self::REMOVE_DEFAULT_PORT && $uri->getPort() !== null && \PYS_PRO_GLOBAL\GuzzleHttp\Psr7\Uri::isDefaultPort($uri)) {
            $uri = $uri->withPort(null);
        }
        if ($flags & self::REMOVE_DOT_SEGMENTS && !\PYS_PRO_GLOBAL\GuzzleHttp\Psr7\Uri::isRelativePathReference($uri)) {
            $uri = $uri->withPath(\PYS_PRO_GLOBAL\GuzzleHttp\Psr7\UriResolver::removeDotSegments($uri->getPath()));
        }
