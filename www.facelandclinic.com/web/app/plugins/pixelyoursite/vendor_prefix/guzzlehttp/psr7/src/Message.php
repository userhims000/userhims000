...
final class Message
{
    ...
    public static function toString(\PYS_PRO_GLOBAL\Psr\Http\Message\MessageInterface $message)
    {
        ...
        } elseif ($message instanceof \PYS_PRO_GLOBAL\Psr\Http\Message\ResponseInterface) {
    ...
    public static function parseMessage($message)
    {
        ...
        $count = \preg_match_all(\PYS_PRO_GLOBAL\GuzzleHttp\Psr7\Rfc7230::HEADER_REGEX, $rawHeaders, $headerLines, \PREG_SET_ORDER);
        // If these aren't the same, then one line didn't match and there's an invalid header.
        if ($count !== \substr_count($rawHeaders, "\n")) {
            // Folding is deprecated, see https://tools.ietf.org/html/rfc7230#section-3.2.4
            if (\preg_match(\PYS_PRO_GLOBAL\GuzzleHttp\Psr7\Rfc7230::HEADER_FOLD_REGEX, $rawHeaders)) {
    ...
    public static function parseRequest($message)
    {
        ...
        $request = new \PYS_PRO_GLOBAL\GuzzleHttp\Psr7\Request($parts[0], $matches[1] === '/' ? self::parseRequestUri($parts[1], $data['headers']) : $parts[1], $data['headers'], $data['body'], $version);
    ...
    public static function parseResponse($message)
    {
        ...
        $parts = \explode(' ', $data['start-line'], 3);
        return new \PYS_PRO_GLOBAL\GuzzleHttp\Psr7\Response((int) $parts[1], $data['headers'], $data['body'], \explode('/', $parts[0])[1], isset($parts[2]) ? $parts[2] : null);
    }
}
