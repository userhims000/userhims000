...
<select
	...
	foreach ( $options as $option ) {
		...
		<option value="<?php echo esc_attr( $option['value'] ); ?>"
			...
			?>
		>
		<?php echo $option['label']; // @codingStandardsIgnoreLine ?>
	</option>
		<?php
	}//end foreach
	?>
		...
		>
			...
			foreach ( $described_options as $option ) {
				...
				<li><span class="description">
					<strong><?php echo $option['label']; // @codingStandardsIgnoreLine ?>.</strong>
					<?php echo $option['desc']; // @codingStandardsIgnoreLine ?>
				</span></li>
				<?php
			}//end foreach
			?>
