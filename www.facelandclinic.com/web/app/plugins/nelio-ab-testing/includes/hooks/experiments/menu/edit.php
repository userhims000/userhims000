...
function maybe_enqueue_assets_for_alternative() {
	...
	$experiment_id  = absint( $_REQUEST['experiment'] ); // phpcs:ignore
	$alternative_id = sanitize_text_field( wp_unslash( $_REQUEST['alternative'] ) ); // phpcs:ignore
...
function maybe_die_if_params_are_invalid() {
	...
	if ( ! isset( $alternative['attributes'] ) || ! isset( $alternative['attributes']['menuId'] ) || $alternative['attributes']['menuId'] !== absint( $_REQUEST['menu'] ) ) { // phpcs:ignore
