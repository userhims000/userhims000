...
class Nelio_AB_Testing_Experiment {
	 ...
	 *
	 * @return boolean|WP_Error whether the experiment can be stopped or not. If it can't, it returns an error with the explanation.
	 *
	 * @since  6.0.1
	 */
	 ...
	 *
	 * @return boolean|WP_Error whether the experiment can be paused or not. If it can't, it returns an error with the explanation.
	 *
	 * @since  6.0.1
	 */
	 ...
	 *
	 * @return boolean|WP_Error whether the experiment can be restarted or not. If it can't, it returns an error with the explanation.
	 *
	 * @since  6.2.0
	 */
	...
	public function get_goals() {
		$this->set_goals( is_array( $this->goals ) ? $this->goals : array() );
		return $this->goals;
	}//end get_goals()
		...
		foreach ( $goals as &$goal ) {
			foreach ( $goal['conversionActions'] as &$action ) {
				...
				if ( 'custom' === $goal_tracking ) {
					...
					$scope = array( 'type' => $goal_tracking );
				}//end if
				 ...
				 * @since 6.0.4
				 */
				$action['attributes'] = apply_filters( 'nab_sanitize_conversion_action_attributes', $attributes, $action );
				 ...
				 * @since 6.0.4
				 */
				$action['scope'] = apply_filters( 'nab_sanitize_conversion_action_scope', $scope, $action );
			}//end foreach
		}//end foreach
	 ...
	 *
	 * @return boolean|WP_Error whether this experiment has been restarted or not.
	 *
	 * @since  6.2.0
	 */
	 ...
	 *
	 * @return boolean whether the experiment is inline or not.
	 *
	 * @since  6.0.0
	 */
	 ...
	 *
	 * @param boolean $active whether the experiment loads alternative content (and therefore the summary needs to contain more data) or not.
	 *
	...
	public function summarize( $active ) {
		$result = array(
			...
			'id'           => $this->get_id(),
			'type'         => $this->get_type(),
			'alternatives' => array_map( '__return_zero', $this->get_alternatives() ),
		...
		if ( 'footer' === $this->get_page_view_tracking_location() ) {
			$result = array_merge( $result, array( 'hasFooterView' => true ) );
		}//end if
		return $result;
	}//end summarize()
