...
class Nelio_AB_Testing_Experiment_List_Page extends Nelio_AB_Testing_Abstract_Page {
	...
	public function maybe_show_admin_notices_regarding_experiment_status_changes() {
		...
		$notice = '<div class="updated notice is-dismissible"><p>%s</p></div>';

		if ( isset( $_GET['nab_started'] ) ) { // phpcs:ignore
			printf( $notice, esc_html_x( 'Test started.', 'text', 'nelio-ab-testing' ) ); // phpcs:ignore
		}//end if

		if ( isset( $_GET['nab_restarted'] ) ) { // phpcs:ignore
			printf( $notice, esc_html_x( 'Test restarted.', 'text', 'nelio-ab-testing' ) ); // phpcs:ignore
		}//end if

		if ( isset( $_GET['nab_paused'] ) ) { // phpcs:ignore
			printf( $notice, esc_html_x( 'Test paused.', 'text', 'nelio-ab-testing' ) ); // phpcs:ignore
		}//end if

		if ( isset( $_GET['nab_resumed'] ) ) { // phpcs:ignore
			printf( $notice, esc_html_x( 'Test resumed.', 'text', 'nelio-ab-testing' ) ); // phpcs:ignore
		}//end if

		if ( isset( $_GET['nab_stopped'] ) ) { // phpcs:ignore
			printf( $notice, esc_html_x( 'Test stopped.', 'text', 'nelio-ab-testing' ) ); // phpcs:ignore
		}//end if

		if ( isset( $_GET['nab_duplicated'] ) ) { // phpcs:ignore
			printf( $notice, esc_html_x( 'Test duplicated.', 'text', 'nelio-ab-testing' ) ); // phpcs:ignore
		}//end if
...
JS;
	...
	private function get_action_url() {
		...
		return add_query_arg(
			array(
				'page'       => 'nelio-ab-experiment-' . sanitize_text_field( wp_unslash( $_GET['action'] ) ), // phpcs:ignore
				'experiment' => absint( $_GET['experiment'] ), // phpcs:ignore
			),
			admin_url( 'admin.php' )
		);
