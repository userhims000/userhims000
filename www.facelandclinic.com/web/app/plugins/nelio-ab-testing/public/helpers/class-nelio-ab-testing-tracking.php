...
class Nelio_AB_Testing_Tracking {
	...
	public function should_experiment_trigger_page_view( $experiment, $location ) {
	...
	}//end should_experiment_trigger_page_view()
	...
	private function get_footer_views() {
		...
		$experiments = array_filter(
			$experiments,
			function( $experiment ) {
				return $this->should_experiment_trigger_page_view( $experiment, 'footer' );
			}
		);
