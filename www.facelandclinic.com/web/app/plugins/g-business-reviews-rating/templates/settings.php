...
<?php if ($this->count_reviews_all >= 1): ?>
        <a href="#reviews" class="nav-tab<?php echo ($this->section == 'reviews') ? ' nav-tab-active' : ''; ?>"><span class="dashicons dashicons-star-filled"></span> <?php esc_html_e('Reviews', 'g-business-reviews-rating'); ?> <span class="count"><?php echo esc_html($this->count_reviews_all); ?></span></a>
<?php endif; ?>
...
?>
                ...
                <tr id="color-schemes">
                    ...
                    <td>
                    	<label class="<?php echo 'default' . ((get_option('google_business_reviews_rating_color_scheme') == NULL) ? ' selected' : ''); ?>" for="color-scheme"><input type="radio" id="color-scheme" name="google_business_reviews_rating_color_scheme" value=""<?php echo (get_option('google_business_reviews_rating_color_scheme') == NULL) ? ' checked' : ''; ?>> <?php _e('None', 'g-business-reviews-rating'); ?></label>
...
<?php /* translators: 1: plugin support URL, 2: author's name, 3: author's website, 4: author's business name */ 
	echo sprintf(__('			<p>This review plugin came about as a side-effect of collecting a business’s opening times using the Places API from Google which sources data from a client’s Google My Business listing. The recent review data is available and, with some tweaks, it could be displayed anywhere in a similar style to the actual reviews popup in the Google search results.</p>
			...
			<p><span class="signature" title="%4$s"></span><br>
				Developer, <a href="%5$s">%6$s</a></p>', 'g-business-reviews-rating'), 'https://designextreme.com/wordpress/', 'https://wordpress.org/support/plugin/g-business-reviews-rating/reviews/#new-post', 'https://designextreme.com/wordpress/', 'Noah H', 'https://designextreme.com', 'Design Extreme'); ?>
