	...
	public function __construct()
	{
		...
		if (!$this->translation_exists())
		{
			...
			case 'fr':
				...
				$this->relative_times['hours']['text'] = '%u hours ago';
...
';
		...
		$translate = (is_bool(get_option(__CLASS__ . '_retrieval_translate', NULL)) && get_option(__CLASS__ . '_retrieval_translate', FALSE));
...
';
		...
		<ul class="subsubsub links">
			<li class="reviews"><a href="' . esc_attr(admin_url('options-general.php?page=google_business_reviews_rating_settings#reviews')) . '">' . __('Reviews', 'g-business-reviews-rating') . ' <span class="count">(<span class="reviews-count">' . esc_html($this->reviews_count()) . '</span>)</span></a> |</li>
			<li class="settings"><a href="' . esc_attr(admin_url('options-general.php?page=google_business_reviews_rating_settings')) . '">' . __('Settings', 'g-business-reviews-rating') . '</a> |</li>
			<li class="about"><a href="' . esc_attr(admin_url('options-general.php?page=google_business_reviews_rating_settings#about')) . '">' . __('About', 'g-business-reviews-rating') . '</a> |</li>
...
';
	...
	public function wp_display($atts = NULL, $content = NULL, $shortcode = NULL)
	{
		...
		$loop = (is_numeric($view) && is_numeric($loop) && $loop >= 1 && $loop <= 999) ? intval($loop) : (is_bool($loop) && $loop || is_string($loop) && preg_match('/^(?:t(?:rue)?|y(?:es)?|1|on|show|loop|infin[ia]te?|forever|always)$/', $loop));
		...
		$bullet = (is_string($bullet) && (mb_strlen($bullet) < 20 && !preg_match('/^(?:false|no(?:ne)?|0|off|hide)$/i', $bullet))) ? $bullet : (!array_key_exists('bullet', $atts) || is_bool($bullet) && $bullet || is_string($bullet) && !preg_match('/^(?:false|no(?:ne)?|0|off|hide)$/i', $bullet));
...
';
				...
				elseif (!is_numeric($limit) || is_numeric($limit) && $limit > 0)
				{
					...
					if (is_string($review_item_order))
					{
						...
						$options['review_item_author_switch'] = (preg_match('/([\b\s,_-]|^)(?:author(?:[\b\s,_-])?)[\b\s,_-]?(?:last|bottom|after|low|below|switch|flip)([\b\s,_-]|$)/i', $review_item_order));
						
						if (!$options['review_item_text_first'] && !$options['review_item_author_switch'] && preg_match('/^(?:(?:author|avatar|date|inline|name|rating|review|text)[,\s]*){2,6}$/i', $review_item_order))
						{
...
';
		...
		if (in_array('author', $review_item_order))
		{
			$check_key = array_search('author', $review_item_order);
			
			if (!in_array('name', $review_item_order))
			{
				$review_item_order[$check_key] = 'name';
			}
			else
