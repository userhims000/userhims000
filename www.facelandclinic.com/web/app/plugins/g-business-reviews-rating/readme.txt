...
Tags: google reviews, google my business, google rating, structured data, gmb reviews, gmb rating
...
3. Use the **Settings**→**Reviews and Rating – Google My Business** screen to configure the plugin with your [Google API Key](https://console.developers.google.com/apis/credentials) and the business’ [Place ID](https://developers.google.com/places/place-id). Here is a copy of the same instructions:
	a. Create a new project or open an existing project in [Google Developer’s Console](https://console.developers.google.com/apis/credentials)
	b. Search for [Places API](https://console.cloud.google.com/apis/library/places-backend.googleapis.com?q=place) and click the button to enable this API in your account
	c. In [Credentials](https://console.developers.google.com/apis/credentials), click the button: “+ Create Credentials”
	d. Select “API Key” from the options
	e. Once this key is created, click “Close”
	f. Select your newly created API Key
	g. Under “Application restrictions”, set this to: “IP addresses” and “Add an item” with your web server’s IP
	h. Under “API restrictions”, select “Restrict Key”, select just “Places API” from the list of options and click “OK”
	i. Click “Save” to set the restrictions
	j. Copy this new API Key to this plugin’s settings
	k. Finally for regular requests, please [enable billing](https://console.cloud.google.com/projectselector/billing/enable) for your project to receive your substantial and free API request allocation
