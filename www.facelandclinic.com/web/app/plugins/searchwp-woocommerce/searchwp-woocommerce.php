...
if ( ! defined( 'SEARCHWP_WOOCOMMERCE_VERSION' ) ) {
	define( 'SEARCHWP_WOOCOMMERCE_VERSION', '1.3.1' );
}
...
class SearchWP_WooCommerce_Integration {
	...
	function __construct() {
		...
		add_filter( 'searchwp\post__not_in', array( $this, 'exclude_hidden_products' ) );
		add_filter( 'searchwp_exclude', array( $this, 'exclude_hidden_products' ) ); // SearchWP 3.x compat.

		// Maybe exclude out of stock products.
		add_filter( 'searchwp\post__not_in', array( $this, 'maybe_exclude_out_of_stock_products' ) );
	...
	function implement_mods( $mods ) {
		...
		$mod->raw_join_sql( $this->query_main_join( '', null ) );

		$column_as = $this->searchwp_query_inject();
		if ( ! empty( $column_as ) ) {
			$mod->column_as( 'average_rating', $column_as );
		}
		...
		if ( $where ) {
			$mod->raw_where_sql( $this->searchwp_query_where() );
		}
	...
	function post_in( $filtered_posts ) {
		...
		if ( $this->is_woocommerce_search()
			// && ! isset( $_GET['orderby'] )
			&& get_search_query() === $this->original_query ) {
