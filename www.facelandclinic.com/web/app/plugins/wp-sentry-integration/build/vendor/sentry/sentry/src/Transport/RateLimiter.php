...
final class RateLimiter
{
    ...
    public function handleResponse(\Sentry\Event $event, \Sentry\HttpClient\Response $response) : \Sentry\HttpClient\Response
    {
    ...
    public function isRateLimited(\Sentry\EventType $eventType) : bool
    {
        $disabledUntil = $this->getDisabledUntil($eventType);
        return $disabledUntil > \time();
    }
    private function getDisabledUntil(\Sentry\EventType $eventType) : int
    {
        $category = (string) $eventType;
        if ($eventType === \Sentry\EventType::event()) {
            $category = 'error';
        }
        return \max($this->rateLimits['all'] ?? 0, $this->rateLimits[$category] ?? 0);
    }
    private function handleRateLimit(\Sentry\HttpClient\Response $response) : bool
    {
        $now = \time();
