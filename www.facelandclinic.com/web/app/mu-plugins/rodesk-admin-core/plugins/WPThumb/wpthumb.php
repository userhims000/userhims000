...
/*-----------------------------------------------------------------------------------*/
/*  !!! This is a custom tweaked version of WPThumb by 12HOOG. Do not edit or update this.
/*-----------------------------------------------------------------------------------*/

include_once( 'wpthumb.crop-from-position.php' );
...
class WP_Thumb {
	...
	public function getCacheFileDirectory() {
		...
		$filename_nice = crc32( serialize( implode( '_', $parts ) ) );
	...
	public function getCacheFileName() {
		...
		$original_filename = pathinfo( $this->getFilePath() );
		$serialize = $original_filename['filename'] . '-' . $this->getArg( 'width' ) . 'x' . $this->getArg( 'height' );
	...
	private function getFileURLForFilePath( $path ) {
		...
		if ( strpos( $path, $upload_dir['basedir'] ) !== false ) {
			...
			$file_url = str_replace( self::get_home_path(), trailingslashit( home_url() ), $path );
		}
...
function wpthumb_add_image_editors( $editors ) {
	...
	require_once( 'wpthumb.image-editor.php' );
