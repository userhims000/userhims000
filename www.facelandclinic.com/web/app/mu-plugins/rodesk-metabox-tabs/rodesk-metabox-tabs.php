<?php
/*
Plugin Name: Rodesk Metabox Tabs
Plugin URI: https://bitbucket.org/web-nl/rodesk-metabox-tabs
Description: Adds extendable metabox tabs to your posts (and specific pages). Based on a plugin by SilbinaryWolf (Ultimate Metabox Tabs)
Version: 1.1.8
Author: Joeri Abbo | Rodesk BV
Author URI: http://www.rodesk.nl
...
class UltimateMetaboxTabs
{
	...
	function __construct()
	{
		if (is_admin()) {
			...
			$this->developer_email = "jasper.r@rodesk.nl";
			$this->version = '0.1';
			...
			$this->extension_database_prefix = "rodesk_extension_metatab_";
			$this->post_database_prefix = "rodesk_post_metatab_";
			$this->option_database_prefix = "rodesk_option_metatab";
			$this->settings_database_prefix = "rodesk_";
			...
			$this->metatabs_page_loaded = false;
	...
	function init()
	{
		...
		// Always run edit_form_advanced_pather by default
		// @TODO find a better method for this
		if ($this->patches['edit_form_advanced_patcher']->status === false) {
			$result = $this->patches['edit_form_advanced_patcher']->install();
		}
		...
		if (defined('WPSEO_VERSION')) {
			...
			$this->add_extension(
				"wpseo-support",
				__("WP SEO (Yoast) Support", "umt"),
				__("Adds WP SEO support. So you can load WP SEO metaboxes inside the tabs", "umt"),
				"umt_wpseo_support",
				$addon_dir . 'wpseo.php'
			);
		}
		...
		if (defined('THE_SEO_FRAMEWORK_VERSION')) {
			...
			$this->add_extension(
				"seoframework-support",
				__("SEO Framework Support", "umt"),
				__("Adds SEO Framework support. So you can load SEO Framework metaboxes inside the tabs", "umt"),
				"umt_seoframework_support",
				$addon_dir . 'seoframework.php'
			);
		}
		...
		if (isset($acf)) {
			...
			$this->add_extension(
				"acf-post-list",
				__("ACF list custom fields", "umt"),
				__("Lists ACF custom fields in a selectable list to the metatab editor.", "umt"),
				"umt_acf_post_list",
	...
	{
		if ($post == "nav_menu_item" || $post == "revision" || $post == "attachment" || $post == "acf" || $post == "acf-field-group" || $post == "acf-field") {
			return true;
		}
		return false;
	}
	...
	function admin_menu()
	{
		add_submenu_page($this->menu_parent, __('Rodesk Metabox Tabs', 'umt'), __('Metabox Tabs', 'umt'), 'administrator', $this->menu_slug, array($this, 'admin_view'));
	}
...
function admin_load()
{
	...
	if ($this->post_type !== null || $this->page_id !== null) {
		if (isset($this->page_id)) {
			$option_name = $this->post_database_prefix . $this->page_id;
		} else {
...
function metatab_load($posttype_slug = null)
{
	...
	if ($this->metatabs_page_loaded != true) {

			// Don't attempt to load if there is no page
		if (get_the_id() !== '') {
				// Setup defaults
			$this->metatabs_page_loaded = true;

			if (isset($_GET['page_id']) || isset($_GET['post'])) {
				if( function_exists( 'icl_object_id' ) && !empty($sitepress) ) {
					global $sitepress;
					$lang = $sitepress->get_default_language();
					$this->page_id = icl_object_id( get_the_id() , 'page', false, $lang );
				} else if (function_exists('pll_get_post')) {
					$current_lang = pll_default_language();
					$this->page_id = pll_get_post(get_the_id(), $current_lang);
				} else {
...
{ ?>
		<div id="sw-ultimate-metabox-tab-list-container">
			<ul id="sw-ultimate-metabox-tab-list" class="metabox-tabs" style="z-index:99;">
				...
				<?php $acf_groups = implode(',', wp_list_pluck($metatab['div'], 'name')); ?>
				<?php $class = ($i == $this->current_tab) ? 'class="active"' : ''; ?>
				<?php $url = add_query_arg('umt_tab', $i); ?>
				<li id="<?php echo $metatab['id']; ?>" data-acf-groups="<?php echo $acf_groups; ?>" style="display:none">
...
function admin_view()
{
	...
	if (isset($_REQUEST['subpage'])) {
	...
	} else if (isset($_REQUEST['posttype']) || isset($_REQUEST['options']) || isset($_REQUEST['settings']) || isset($_REQUEST['page_id'])) {
		...
		if (isset($_REQUEST['posttype'])) {
			...
			$this->settings_type = $_REQUEST['page_id'];
